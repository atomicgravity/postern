...
The CLI sends the IdP-issued **access token** to the broker (`Authorization: Bearer`). The ID token is local-display-only — it tells the CLI who the engineer is for the "Logged in as ..." line and is never sent to the broker. **Never validate ID tokens at the broker** — they're meant for the client (CLI), not for resource servers, and accepting them as broker auth opens cross-app token misuse where a token issued for App A can be replayed against App B if both share a client_id. The broker validates `aud` and/or `scope` against its config to ensure the token was issued *for this broker*. The IdP doesn't enforce this — the broker does. "and/or" is literal: with only one of `idp.audience` / `idp.required_scope` configured, that one is required; with **both** configured, **either** match suffices (OR) — a token needs at least one of the two configured proofs, and a token with neither is rejected (cross-app isolation is preserved). This lets one config serve a single IdP whose human tokens carry `aud` (RFC 8707 resource binding) and whose machine/client-credentials tokens carry only a scope. **Don't restore AND-both semantics or add a match-mode knob speculatively** — strict-AND across a single IdP's two caller classes is the exact case OR exists to serve; a real operator need would be a future explicit knob, not a v1 default.

### The APIGW JWT authorizer pins audience, not scope

When `apigw_jwt_authorizer_enabled` puts an API Gateway HTTP API JWT authorizer in front of the broker Lambda, it is a defense-in-depth pre-filter (signature + issuer + exp + audience), not the authority — the broker re-verifies every token. The edge authorizer can only AND its configured constraints, so it **can't express the broker's aud-OR-scope**. It therefore pins an audience list — `idp_audience` plus `apigw_jwt_additional_audiences` — and relies on APIGW matching a token's `aud` claim, or its `client_id` claim when `aud` is absent. Client-credentials (machine) tokens carry no `aud`, so they're admitted by listing their client IDs in `apigw_jwt_additional_audiences`. **Don't drop the edge audience pin to let machine tokens through, and don't pin scope at the edge instead** — either move breaks one of the two caller classes, because the authorizer can't OR them. List the machine client IDs; the broker stays the authoritative aud-OR-scope verifier and cross-app isolation lives there regardless. The edge is purely a pre-filter — its audience match is coarse, and removing it loses no security the broker doesn't already enforce.
...
Per `DESIGN.md`. Durable design docs are indexed at `docs/design/index.md`; locked invariants live in `docs/design/constraints.md`.

- `cmd/broker/` — the long-running broker HTTP server: SSH endpoints (`/ssh/cert`, `/ssh/time-payload`, `/ssh/tunnel`), the operational endpoint `/healthz`, and a `--print-config` flag mode for resolved-config debugging. `/ssh/tunnel` returns 501 only when the deployment has no `tunneling:` section configured — the architectural opt-in path, not a placeholder.
- `cmd/broker-lambda/` — the API-Gateway-fronted Lambda entrypoint. Shares the handler stack and dep wiring with `cmd/broker`; consumed by the `terraform/postern-broker/` module.
- `cmd/postern/` — the CLI: `login`, `mint`, `ssh`, `scp`, `add-host`, `remove-host`, `cache` (`ls`/`prune`), `tunnel`, `timefix`, `setup-ssh`, `logout`, `configure`, `version`, and `upgrade` (placeholder — the signed-release fetch + Sigstore verification subcommand is the one v1 surface still to land).
- `cmd/timefix-apply/` and `cmd/timefix-set-clock/` — the on-device verifier and setter.
- `internal/` — shared internals plus the default impls of all abstractions: `atomicfile`, `audit`, `broker`, `brokerclient`, `brokerwire`, `certcache`, `idp`, `logging`, `oauthlogin`, `policy`, `ratelimit`, `registry`, `securetunnel`, `signer`, `sshconf`, `tokenstore`, `tunneling`, `version`.
- `pkg/` — the wrapper composition surface: `brokerhandlers`, `cliapp`.
- `terraform/postern-broker/` — the IaC reference module: KMS, DynamoDB, AVP with Cognito + generic OIDC identity-source variants and an operator-customizable schema/policy set, CloudWatch, IAM, Lambda, API Gateway HTTP API, optional Route 53 + ACM custom domain, optional pre-built Lambda zip path, configurable HTTP-registry timeout.
- `examples/` — reference packaging artifacts, not framework concerns: `examples/terraform/cognito/` (Cognito IdP sample), `examples/terraform/deployment/` (module consumer), `examples/on-device/sshd/` (partial sshd_config drop-in), `examples/on-device/timefix/`, `examples/on-device/secure-tunnel/` (device-side tunnel destination reference). A principals-init script and distro packaging samples (Yocto, Buildroot) are not yet authored.
- The release pipeline: `.goreleaser.yaml` + GitHub Actions (`test.yml`, `release.yml`) + `release-please-config.json`. release-please opens a release PR on every `feat:`/`fix:` push to main; merging it tags `v*` and chains GoReleaser, which publishes a GitHub Release with cosign-keyless-signed checksums via Sigstore Fulcio.
- The pure-Go implementation of the AWS V3 secure-tunneling source proxy behind the AWS IoT Tunneling default impl. The protocol is documented at https://github.com/aws-samples/aws-iot-securetunneling-localproxy/blob/main/V3WebSocketProtocolGuide.md; a community Go reference implementation exists at https://github.com/mizosukedev/securetunnel for cross-checking.
